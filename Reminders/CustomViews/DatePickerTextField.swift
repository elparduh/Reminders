...
struct DatePickerTextField: UIViewRepresentable {
    ...
    func makeUIView(context: Context) -> UITextField {
        ...
        let doneButton = UIBarButtonItem(title: "Aceptar", style: .plain, target: helper, action: #selector(helper.doneButtonTapped))
    ...
    func updateUIView(_ uiView: UITextField, context: Context) {
        if let selectedDate = date {
            uiView.text = HelperFormatter.dateFormatter.string(from: selectedDate)
        }
    }
